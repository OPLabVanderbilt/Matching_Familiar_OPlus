...
% modified by Mackenzie Aug 2017 to run for OPlus with familiar objects
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [dprime timeout] = matching_Plane(subjno,subjini,age,sex,hand)
try
    %% Open Screen
    ...
    %% make mask
    if category == 'b'
        mask = imread('Bird_mask.jpg');
    elseif category == 'p'
        mask = imread('Plane_mask.jpg');
    elseif category == 't'
        mask = imread('Trans_mask.jpg');
    end
    ...
    %%Practice trials
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [cat trial samediff viewpoint S1_name S2_name] = textread(['trials_' category '_practice.txt'], '%s %s %s %s %s %s');
    ...
    n_practrials = length(trial); %should get the number of trials
    for i = 1:n_practrials
        %blank 500ms
        ...
        %S1 300ms or 150ms
        S1 = imread(['prac_stimuli/' char(S1_name(i))]);
        S1 = imresize(S1, [300 300]);
        ...
        S2 = imread(['prac_stimuli/' char(S2_name(i))]);
        S2 = imresize(S2, [300 300]);
        ...
        responsecode = 0;
         ...
         %record response time
            ...
            %print data
            fprintf(dataFile1, '%i\t%i\t%s\t%s\t%s\t%s\t%s\t%s\t%i\t%f\n', str2num(subjno), i, category, char(samediff(i)), char(viewpoint(i)), char(S1_name(i)), char(S2_name(i)), char(resp), GradedRes, rt);
    ...
    %%Experimental Trials
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [cat trial samediff viewpoint S1_name S2_name] = textread(['trials_' category '.txt'], '%s %u %s %s %s %s');
