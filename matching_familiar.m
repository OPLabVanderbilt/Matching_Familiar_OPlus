...
%function [] = matching_test(subjno,subjini,age,sex,hand)
try
    %% Open Screen
    ...
    for i = 1:n_practrials
        ...
        responsecode = 0;
         ...
         %record response time
            ...
            %print data
            fprintf(dataFile1, '%d\t%i\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%i\t%f\n', str2double(subjno), i, num2str(category), char(samediff(i)), char(viewpoint(i)), char(S1_name(i)), char(S2_name(i)), char(resp), GradedRes, rt);
